...
void receive_position_packets(sf::UdpSocket &socket, std::vector<player> &players, std::vector<bullet> &bullets, bool &playerWalkingReceived, bool &bulletCreatedReceived)
{
    ...
    for(int i=0; i<3; ++i)
    {
        ...
        receive_bullet_created(posPacket, bullets, players, bulletCreatedReceived);
    }
}
...
void send_clients_bullet_created_to_all_clients(const sf::IpAddress ip, std::vector<unsigned short> ports,const bool bulletCreatedReceived)
{
